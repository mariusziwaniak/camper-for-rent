DROP TABLE IF EXISTS payment CASCADE;
DROP TABLE IF EXISTS invoice CASCADE;
DROP TABLE IF EXISTS payment_type CASCADE;
DROP TABLE IF EXISTS invoice_type CASCADE;
DROP TABLE IF EXISTS booking CASCADE;
DROP TABLE IF EXISTS customer CASCADE;
DROP TABLE IF EXISTS price CASCADE;
DROP TABLE IF EXISTS exclusion CASCADE;
DROP TABLE IF EXISTS vehicle_detail CASCADE;
DROP TABLE IF EXISTS detail CASCADE;
DROP TABLE IF EXISTS vehicle CASCADE;
DROP TABLE IF EXISTS category CASCADE;
DROP TABLE IF EXISTS owner CASCADE;
DROP TABLE IF EXISTS address CASCADE;
DROP TABLE IF EXISTS camper_for_rent_user_role CASCADE;
DROP TABLE IF EXISTS camper_for_rent_user CASCADE;
DROP TABLE IF EXISTS camper_for_rent_role CASCADE;
DROP TABLE IF EXISTS flyway_schema_history CASCADE;
